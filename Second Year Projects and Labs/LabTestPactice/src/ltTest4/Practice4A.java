...
public class Practice4A {
    ...
    public static String removeA(String s) {
    	...
    	if(s.startsWith("A"))
    	{
    		return removeA(s.substring(1, s.length()));
    	}
    	if (s.endsWith("A"))
    	{
    		return removeA(s.substring(0, s.length() -1));
    	}
       	return	 String.valueOf(s.charAt(0)) + removeA(s.substring(1, s.length()));
    ...
    public static int sumSkip(List<Integer> t) {
    	...
    	return t.get(0) + sumSkip(t.subList(2, t.size()));
    }
    ...
    private static int maxSum(List<Integer> t, int n, int currentSum) {
    	...
    	int first = t.get(0);
    	int with = currentSum;
    	if(first + currentSum <= n)
    	{
    		with = maxSum(t.subList(1, t.size()), n, first + currentSum);
    	}
    	int without = maxSum(t.subList(1, t.size()), n, currentSum);
